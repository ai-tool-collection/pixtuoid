...
| `[audio]` | Ambient office sound — **off unless you opt in** (`enabled = true`; a terminal app never speaks uninvited). `volume` (0.0–1.0, clamped, default 1.0) scales everything. When enabled: a lofi office soundscape mixed live from how busy the office is (typing density tracks active agents), gentle rain when the office weather rains, plus one-shots — elevator ding on floor navigation, a door chime on walk-ins, printer/vending/water-cooler moments. Press `m` in the TUI to mute/unmute for the session. Every sound is synthesized at startup (no audio files). Prebuilt **Linux** binaries ship without audio (ALSA can't link into the static/cross builds); building from source on Linux enables it when the ALSA headers (`libasound2-dev`) are present. |
