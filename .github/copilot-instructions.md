...
4. Hook install (`install::install_target`, driven by the in-TUI Connection panel `c` — no `install-hooks` CLI) writes through symlinks (`resolve_symlink`) — don't replace with `fs::rename`.
